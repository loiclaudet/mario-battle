...
the sample on the raw waveform. writes assets/music/battle_intro.flac, battle_loop.flac, tools/music_cuts.json, and patches the two
constants in scripts/sfx.luau.
run from the project root: python3 tools/cut_music.py            (automatic analysis)
                           python3 tools/cut_music.py 5 19       (manual: intro ends at 5 s, loop ends at 19 s)"""
...
if manual:
    S = int(round(float(sys.argv[1]) * sr))
    L = int(round(float(sys.argv[2]) * sr)) - S
    print(f'manual cut: intro {S / sr:.3f} s, loop {L / sr:.3f} s')

# 1. period: best envelope self-similarity for a loop between 8 and 20 s
e10 = env(0, len(x), 441)
best = (0, 0)
for L in (np.arange(8.0, 20.0, 0.01) if not manual else []):
...
seg = x[a0:a0 + W]
bc, bL = -1, L
for Ls in (range(L - 2000, L + 2000, 10) if not manual else []):
    seg2 = x[a0 + Ls:a0 + Ls + W]
    c = np.dot(seg, seg2) / (np.linalg.norm(seg) * np.linalg.norm(seg2))
    if c > bc:
        bc, bL = c, Ls
for Ls in (range(bL - 12, bL + 12) if not manual else []):
    seg2 = x[a0 + Ls:a0 + Ls + W]
    c = np.dot(seg, seg2) / (np.linalg.norm(seg) * np.linalg.norm(seg2))
    if c > bc:
        bc, bL = c, Ls
...
for i in cands:
    ...
    if start + 2 * L > len(x):
        continue
    c = np.corrcoef(env(start, start + L, 441), env(start + L, start + 2 * L, 441))[0, 1]
...
sfx_path = os.path.join(root, 'scripts/sfx.luau')
sfx = open(sfx_path).read()
sfx = re.sub(r'Sfx\.INTRO_SECONDS = .*', f'Sfx.INTRO_SECONDS = {S} / {sr}', sfx)
sfx = re.sub(r'Sfx\.LOOP_SECONDS = .*', f'Sfx.LOOP_SECONDS = {L} / {sr}', sfx)
open(sfx_path, 'w').write(sfx)
print(f'wrote assets/music/battle_intro.flac ({S / sr:.3f} s) and battle_loop.flac ({L / sr:.3f} s), constants patched in scripts/sfx.luau')
